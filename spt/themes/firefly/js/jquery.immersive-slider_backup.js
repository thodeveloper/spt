  ...
  $.fn.immersive_slider = function(options){
    ...
    if (settings.cssBlur == true) {
      ...
      el.find(".is-slide").each(function( index ) {
        var activeclass = ""
        if(index == 0) activeclass = "active"
       // var img = "<img src='"+ $(this).data("blurred") +"'>"; edit clear bg
	   var img = "";
    ...
    el.find(".is-slide").each(function( index ) {
      ...
      if(settings.pagination == true) {
        pagination += "<li><a class='is-page phone" + (index + 1) + " " + activeclass + "' href='#slide_" + (index + 1) + "'></a></li>"
      }
    });
    
    $("<ul class='is-pagination'>"+pagination+"</ul>").appendTo('.is-container')
